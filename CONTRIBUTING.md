...
| | |
|---|---|
| `internal/verify/` | 34 Go tests — static checks over the current source. Picked up by `go test ./...`; nothing lists them. |
| `web/test/` | 22 test files that bundle the app's TypeScript and run it against a DOM shim, via `npm test`. |
