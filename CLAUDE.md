...
# The frontend's own tests: 30 of them, bundling the app's TypeScript with
...
| | |
|---|---|
| `internal/verify/` | 46 Go tests. Static checks over the CURRENT source: credentials, cited paths, the WebSocket vocabulary both ways, endpoints, selectors, module reachability, identity columns, the blur-suspend guard, the fast/slow poll ledger, the shared-menu ledger, fixture schemas, and that every page-key literal still names a real page. |
...
- **The two gates are not unit tests.** `cmd/conformance` and `cmd/compat` run against live hardware
  ...
  them.
- **`internal/verify/`** holds the repository's static self-checks as Go tests — 46 of them. They
...
- **`web/test/`** holds 30 frontend tests that bundle the app's TypeScript and run it against a DOM
